...
def curveAdaptorCode : Pipeline.CurveAdaptor → Nat
  | .y4 => 0 | .y6 => 1 | .x3 => 2 | .x5 => 3 | .x7 => 4

def pointCoordinateCode : Pipeline.PointCoordinate → Nat
  | .x => 0 | .y => 1 | .z => 2
...
def coordinateKey (seed : BitVec 256) (base : Nat) : CoordinateMacKey :=
  Vector.ofFn fun index =>
    { falseLabel := block seed (base + 2 * index.val)
      trueLabel := block seed (base + 2 * index.val + 1) }
...
def rowRandomness (seed : BitVec 256) (row : Nat) : FieldMacToECMac.RowRandomness :=
  ...
  { rho := nonZeroBase (field seed base)
    x := {
      ...
      r3 := field seed (base + 3)
      r5 := field seed (base + 4) }
    y := {
      r1 := field seed (base + 5)
      r4 := field seed (base + 6)
      r5 := field seed (base + 7) }
    z := {
      r2 := field seed (base + 8)
      r3 := field seed (base + 9)
      r4 := field seed (base + 10)
      r5 := field seed (base + 11) } }
...
def randomness (seed : BitVec 256) : Garbling.Randomness := {
  ...
  fixedKeyOracle := ⟨fun index => shift (block seed (10_000 + fixedKeyCode index))⟩
  inputMacKey := { x := coordinateKey seed 0, y := coordinateKey seed 600 }
