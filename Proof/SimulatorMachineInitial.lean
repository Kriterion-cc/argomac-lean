import Proof.SimulatorMachineLaw
...
def withOracles (metadata : Metadata) (coin : SimulatorOracleCoin) : SimulatorState where
  fixedOracle := coin.fixedOracle
  encOracle := coin.encOracle
  hashOracle := coin.hashOracle
...
def oracleCoinEquiv :
    ((OracleIndex → Equiv.Perm (Fin (2 ^ 128))) × (BaseField → Fin (2 ^ 256))) ≃ SimulatorOracleCoin where
  toFun sample := {
    fixedOracle := ⟨fun index => blockPermEquiv (sample.1 (.inl index))⟩
    encOracle := ⟨fun index => blockPermEquiv (sample.1 (.inr index))⟩
    hashOracle := fun input => hashFin.symm (sample.2 input)
  }
  invFun coin :=
    (fun index => match index with
      | .inl index => blockPermEquiv.symm (coin.fixedOracle.permutation index)
      | .inr index => blockPermEquiv.symm (coin.encOracle.permutation index),
      fun input => hashFin (coin.hashOracle input))
  left_inv sample := by
    apply Prod.ext
    · funext index
...
theorem initial_completion (metadata : Metadata) :
  ...
  have law := congrArg (fun distribution : PMF SimulatorOracleCoin =>
