...
private theorem uniform_map_equiv {A B : Type*}
    [Fintype A] [Fintype B] [Nonempty A] [Nonempty B] (e : A ≃ B) :
    (PMF.uniformOfFintype A).map e = PMF.uniformOfFintype B := by
  classical
  apply PMF.ext
  intro b
  rw [PMF.map_apply]
  simp only [PMF.uniformOfFintype_apply, ← e.symm_apply_eq, eq_comm]
  rw [tsum_ite_eq, Fintype.card_congr e]
