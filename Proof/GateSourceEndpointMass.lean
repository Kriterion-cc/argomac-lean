...
theorem gateSourcePhases_eq {Aux : Type}
    ...
    ((gateSourceChoose adversary parameter auxiliary table data).bind fun selected =>
      gateSourceObserve adversary parameter auxiliary table selected (view selected.1) data) =
    (twoPhaseTranscript idealOracleHandler (adversary.chooseInput parameter table auxiliary)
      (fun state selected => PMF.pure
        (sourceInputLabels data selected.1,
          programSelectedGateView state selected.1 (sourceInputLabels data selected.1).inputMac (view selected.1)))
      (fun selected labels => adversary.decide parameter table labels auxiliary selected.2)
      (initialSourceOracle data)).map (Prod.mk table) := by
  simp only [gateSourceChoose, gateSourceObserve, twoPhaseTranscript, PMF.bind_map,
...
theorem gateSourcePhases_mass_factor {Aux : Type}
    (adversary : GarbledCircuit.AdaptiveAdversary Garbling.oracleSpec AffineInput Pipeline.Table
      Garbling.Labels Aux) (parameter : Nat) (auxiliary : Aux)
    (table : Pipeline.Table) (data : GarblingOracleData) (view : AffineInput → SelectedGateView)
    ...
    (if OracleTranscriptCompatible idealOracleHandler (initialSourceOracle data) before ∧
  ...
  rw [gateSourcePhases_eq, tagged_mass, twoPhaseTranscript_mass_factor
    ...
    (fun state selected => PMF.pure
      (sourceInputLabels data selected.1,
        programSelectedGateView state selected.1 (sourceInputLabels data selected.1).inputMac (view selected.1)))
    (fun selected labels => adversary.decide parameter table labels auxiliary selected.2)
    (initialSourceOracle data) referenceBefore referenceAfter selected labels decision before after
