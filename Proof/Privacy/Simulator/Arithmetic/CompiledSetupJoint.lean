...
theorem lazySetupJoint_machine [FieldCertificate]
    ...
    (simulatorMemoryRun lazyCompiledMachine lazyCompiledMachine.firstFuel
      {bits := fun index => if index = 0 then [false, false] ++ natural parameter ++ natural 9806076 else []}
  ...
  let memory : Memory := {bits := fun index =>
    if index = 0 then [false, false] ++ natural parameter ++ natural 9806076 else []}
  have wire : memory.bits 0 = false :: false :: (natural parameter ++ natural 9806076) := by simp [memory]
  have dispatched : phaseDispatchMemory memory (natural parameter ++ natural 9806076) = compiledSetupBase parameter := by
...
theorem lazySetupJoint_public [FieldCertificate] (parameter : Nat) (memory : Memory)
    (coin : SimulatorSampling.OfflineCoin) (supported : (memory, coin) ∈ (lazySetupJoint parameter).support) :
    publicValue Wire.encoding 9806076 (memory.bits 3) = some (publicSourceTable coin.1) := by
  obtain ⟨result, member, same⟩ := (PMF.mem_support_map_iff _ _ _).mp supported
  cases same
  have actual : result.1 ∈ (samplerBatchMemory offlinePlan 256 917470 0
  ...
  have stored := offlineSourceJoint_words 256 (offlineInitialMemory (compiledSetupBase parameter))
    result.1.1 result.1.2 result.2 member
  have pointer := samplerBatchMemory_caller offlinePlan 256 917470 0
    (offlineInitialMemory (compiledSetupBase parameter)) result.1.1 result.1.2 11 (by decide) actual
  have bitsSaved := samplerBatchMemory_bits offlinePlan 256 917470 0
