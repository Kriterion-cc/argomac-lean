...
theorem lazyOnlineMachine_curveSetup_run [BN254.FieldCertificate] (fuel : Nat) (memory : Memory)
    ...
    lazyOnlineMachine.run (3 + fuel) ⟨1568228, memory⟩ oracle =
      (lazyOnlineMachine.run fuel ⟨1568231, executeLinear onlineOriginalSetup memory⟩ oracle).map
  ...
  exact linear_continue lazyOnlineMachine.arithmetic onlineOriginalSetup
    (onlineBodyLabels 1568228 3 (by decide) 1568231)
...
theorem lazyOnlineMachine_pointSetup_run [BN254.FieldCertificate] (fuel : Nat) (memory : Memory)
    (oracle : LazyOracle.State Shared.FixedKeyIndex EncPRF.PermutationIndex) :
    lazyOnlineMachine.run (3 + fuel) ⟨2883954, memory⟩ oracle =
      (lazyOnlineMachine.run fuel ⟨2883957, executeLinear onlinePointSetup memory⟩ oracle).map
        (Option.map fun result => (result.1, result.2.1, result.2.2 + 3)) := by
  apply Simulator.run_prefix lazyOnlineMachine 3 fuel _ _ oracle
  exact linear_continue lazyOnlineMachine.arithmetic onlinePointSetup
    (onlineBodyLabels 2883954 3 (by decide) 2883957)
...
theorem lazyOnlineMachine_nullFinish [BN254.FieldCertificate] (fuel : Nat) (memory : Memory)
    ...
    lazyOnlineMachine.run (200666 + fuel) ⟨2883951, memory⟩ oracle =
      PMF.pure (some (⟨317804843, onlineFinalMemory (onlineTagMemory memory)⟩, oracle, 200666)) := by
...
theorem lazyOnlineMachine_nullGates [BN254.FieldCertificate] (fuel : Nat) (memory : Memory)
    (oracle : LazyOracle.State Shared.FixedKeyIndex EncPRF.PermutationIndex)
    (absent : memory.ram (BitVec.ofNat 256 (onlineInputBase + 2)) = 0) :
    lazyOnlineMachine.run (117 * 1270 + 200666 + fuel) ⟨1568231, memory⟩ oracle =
      match lazyGateLoopResults curveGatePlan 1270 0 memory oracle with
      | none => PMF.pure none
      | some result => PMF.pure (some
          (⟨317804843, onlineFinalMemory (onlineTagMemory result.1)⟩, result.2.1, result.2.2 + 200666)) := by
  ...
  change lazyOnlineMachine.run (117 * 1270 + (200666 + fuel)) ⟨1568231, memory⟩ oracle = _ at continued
  ...
  | some result =>
    ...
    change (lazyOnlineMachine.run (117 * 1270 + (200666 + fuel) - result.2.2)
      ⟨2883951, result.1⟩ result.2.1).map _ = _
...
theorem lazyOnlineMachine_pointGates_finish [BN254.FieldCertificate] (fuel : Nat) (memory : Memory)
    (oracle : LazyOracle.State Shared.FixedKeyIndex EncPRF.PermutationIndex) :
    lazyOnlineMachine.run (117 * 303784 + 200663 + fuel) ⟨2883957, memory⟩ oracle =
      match lazyGateLoopResults pointGatePlan 303784 0 memory oracle with
      | none => PMF.pure none
      | some result => PMF.pure (some
          (⟨317804843, onlineFinalMemory result.1⟩, result.2.1, result.2.2 + 200663)) := by
  exact loopFinish lazyOnlineMachine pointGatePlan
    (fun pc => onlineBranchLabels 2883957 314720224 (by decide) 317604181 pc.val)
    lazyOnlineMachine_pointGates 2883957 317604181 317804843 (by decide) (by decide)
...
private theorem pointBranchWith [BN254.FieldCertificate]
    ...
    (pointRun : ∀ fuel memory oracle,
      lazyOnlineMachine.run (117 * 303784 + 200663 + fuel) ⟨2883957, memory⟩ oracle =
        ...
        | none => PMF.pure none
        | some result => PMF.pure (some (⟨317804843, onlineFinalMemory result.1⟩, result.2.1, result.2.2 + 200663)))
    (fuel : Nat) (memory : Memory)
    (oracle : LazyOracle.State Shared.FixedKeyIndex EncPRF.PermutationIndex)
    (present : memory.ram (BitVec.ofNat 256 (onlineInputBase + 2)) ≠ 0) :
    lazyOnlineMachine.run (117 * 303784 + 200669 + fuel) ⟨2883951, memory⟩ oracle =
      match source (executeLinear onlinePointSetup (onlineTagMemory memory)) oracle with
      | none => PMF.pure none
      | some result => PMF.pure (some
          (⟨317804843, onlineFinalMemory result.1⟩, result.2.1, result.2.2 + 200669)) := by
  rw [show 117 * 303784 + 200669 + fuel = 3 + (3 + (117 * 303784 + 200663 + fuel)) by omega,
    lazyOnlineMachine_secondBranch_continue]
...
theorem lazyOnlineMachine_pointBranch [BN254.FieldCertificate] (fuel : Nat) (memory : Memory)
    ...
    (present : memory.ram (BitVec.ofNat 256 (onlineInputBase + 2)) ≠ 0) :
    lazyOnlineMachine.run (117 * 303784 + 200669 + fuel) ⟨2883951, memory⟩ oracle =
      match lazyGateLoopResults pointGatePlan 303784 0
        (executeLinear onlinePointSetup (onlineTagMemory memory)) oracle with
      | none => PMF.pure none
      | some result => PMF.pure (some
          (⟨317804843, onlineFinalMemory result.1⟩, result.2.1, result.2.2 + 200669)) := by
  exact pointBranchWith (lazyGateLoopResults pointGatePlan 303784 0)
...
def lazyOnlineValidGateResult (memory : Memory)
  ...
  let point ← lazyGateLoopResults pointGatePlan 303784 0
...
private theorem validGatesWith [BN254.FieldCertificate]
    ...
    (pointRun : ∀ fuel memory oracle,
      ...
      lazyOnlineMachine.run (117 * 303784 + 200669 + fuel) ⟨2883951, memory⟩ oracle =
        match source (executeLinear onlinePointSetup (onlineTagMemory memory)) oracle with
        | none => PMF.pure none
        | some result => PMF.pure (some (⟨317804843, onlineFinalMemory result.1⟩, result.2.1, result.2.2 + 200669)))
    (fuel : Nat) (memory : Memory)
    (oracle : LazyOracle.State Shared.FixedKeyIndex EncPRF.PermutationIndex)
    (present : memory.ram (BitVec.ofNat 256 (onlineInputBase + 2)) ≠ 0) :
    lazyOnlineMachine.run (117 * 305054 + 200672 + fuel) ⟨1568228, memory⟩ oracle =
      PMF.pure ((do
        let curve ← lazyGateLoopResults curveGatePlan 1270 0 (executeLinear onlineOriginalSetup memory) oracle
        ...
        (fun result : Memory × LazyOracle.State Shared.FixedKeyIndex EncPRF.PermutationIndex × Nat =>
          (⟨317804843, result.1⟩, result.2.1, result.2.2))) := by
  let initial := executeLinear onlineOriginalSetup memory
  rw [show 117 * 305054 + 200672 + fuel =
    3 + (117 * 1270 + (117 * 303784 + 200669 + fuel)) by omega,
    lazyOnlineMachine_curveSetup_run]
  have continued := lazyGateLoopBlock_continue lazyOnlineMachine curveGatePlan
    (fun pc => onlineBranchLabels 1568231 1315720 (by decide) 2883951 pc.val)
    lazyOnlineMachine_curveGates 1270 0 (117 * 303784 + 200669 + fuel) initial oracle (by decide)
  change lazyOnlineMachine.run (117 * 1270 + (117 * 303784 + 200669 + fuel)) ⟨1568231, initial⟩ oracle = _ at continued
  rw [continued]
  ...
  | some curve =>
    ...
    change ((lazyOnlineMachine.run (117 * 1270 + (117 * 303784 + 200669 + fuel) - curve.2.2)
      ⟨2883951, curve.1⟩ curve.2.1).map _).map _ = _
    rw [show 117 * 1270 + (117 * 303784 + 200669 + fuel) - curve.2.2 =
      117 * 303784 + 200669 + (117 * 1270 + fuel - curve.2.2) by omega,
...
theorem lazyOnlineMachine_validGates [BN254.FieldCertificate] (fuel : Nat) (memory : Memory)
    ...
    lazyOnlineMachine.run (117 * 305054 + 200672 + fuel) ⟨1568228, memory⟩ oracle =
      PMF.pure ((lazyOnlineValidGateResult memory oracle).map
        (fun result => (⟨317804843, result.1⟩, result.2.1, result.2.2))) := by
  exact validGatesWith (lazyGateLoopResults pointGatePlan 303784 0)
...
theorem lazyOnlineMachine_linked [BN254.FieldCertificate] (fuel : Nat) (memory : Memory)
    ...
    lazyOnlineMachine.run (84355 + (117 * 305054 + 200672) + fuel) ⟨1560762, memory⟩ oracle =
      (lazyEncLinkSamples memory oracle).bind (fun linked => PMF.pure
        ((lazyOnlineValidGateResult linked.1.1 linked.2).map
          (fun result => (⟨317804843, result.1⟩, result.2.1, result.2.2 + linked.1.2.1)))) := by
  have continued := simulator_encLink_samples lazyOnlineMachine
    (fun pc => onlineBranchLabels 1560762 7466 (by decide) 1568228 pc.val)
    lazyOnlineMachine_link memory oracle (117 * 305054 + 200672 + fuel)
  change lazyOnlineMachine.run (84355 + (117 * 305054 + 200672 + fuel)) ⟨1560762, memory⟩ oracle = _ at continued
  rw [Nat.add_assoc, continued]
  ...
  rw [facts.1]
  change (lazyOnlineMachine.run (84355 + (117 * 305054 + 200672 + fuel) - linked.1.2.1)
    ⟨1568228, linked.1.1⟩ linked.2).map _ = _
  rw [show 84355 + (117 * 305054 + 200672 + fuel) - linked.1.2.1 =
    117 * 305054 + 200672 + (84355 + fuel - linked.1.2.1) by omega,
...
theorem lazyOnlineMachine_validBody [BN254.FieldCertificate] [BN254.GroupCertificate]
    ...
    lazyOnlineMachine.run (1 + (onlineSamplingBudget 256 + 1678841 + 84355 +
      (117 * 305054 + 200672)) + fuel) ⟨13621, memory⟩ oracle =
  ...
  rw [show 1 + (onlineSamplingBudget 256 + 1678841 + 84355 + (117 * 305054 + 200672)) + fuel =
    1 + (onlineSamplingBudget 256 + (1678841 + 84355 + (117 * 305054 + 200672) + fuel)) by omega,
  ...
  have rest : onlineSamplingBudget 256 + (1678841 + 84355 + (117 * 305054 + 200672) + fuel) - sampled.2 =
      onlinePrepareCost coin + (84355 + (117 * 305054 + 200672) +
