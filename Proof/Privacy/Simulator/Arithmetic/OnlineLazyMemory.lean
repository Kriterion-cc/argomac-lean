...
structure LazyOnlineGateData [FieldCertificate] [GroupCertificate]
    ...
    (sample : (Fin 91 → Point) × (Fin 92 → NonZeroBase)) (mac : InputMac) (memory : Memory) : Prop where
...
theorem eagerOnline_gateData [FieldCertificate] [GroupCertificate]
  ...
  · have record := onlinePreparedMemory_curveRecord 256 memory sampled.1 sampled.2 coin input output sample stored supported
    apply record.congr _ _ _ prepared.ram linked.ram (BitVec.ofNat 256 privateBase) 913657
    intro offset bound
    rw [← BitVec.ofNat_add]
...
theorem LazyOnlineGateData.frame [FieldCertificate] [GroupCertificate]
    {coin : OfflineCoin} {input : AffineInput} {output : Point}
    {sample : (Fin 91 → Point) × (Fin 92 → NonZeroBase)} {mac : InputMac} {memory next : Memory}
  ...
  · apply data.curve.congr _ _ _ memory.ram next.ram (BitVec.ofNat 256 privateBase) 913657
    intro offset bound
    rw [← BitVec.ofNat_add]
...
theorem LazyOnlineGateData.pointPlan [FieldCertificate] [GroupCertificate]
    {coin : OfflineCoin} {input : AffineInput} {output : Point}
    {sample : (Fin 91 → Point) × (Fin 92 → NonZeroBase)} {mac : InputMac} {memory : Memory}
    (ready : LazyOnlineGateData coin input output sample mac memory) (index : Fin 303784) :
