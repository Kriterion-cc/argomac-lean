...
theorem lazyOnlineMachine_finish_arithmetic [BN254.FieldCertificate] (fuel : Nat) (memory : Memory) :
    run lazyOnlineMachine.arithmetic (200663 + fuel) ⟨317604181, memory⟩ =
      PMF.pure (some (⟨317804843, onlineFinalMemory memory⟩, 200663)) := by
  apply finishRun lazyOnlineMachine.arithmetic 317604181 317604183 317804843 memory
    (executeLinear onlineLabelSetup memory) (onlineFinalMemory memory) fuel 2 200660
  · intro reserve
  ...
  · rw [lazyOnlineMachine_private 317804843 (by decide)]
...
theorem lazyOnlineMachine_finish [BN254.FieldCertificate] (fuel : Nat) (memory : Memory)
    (oracle : Cryptography.LazyOracle.State Shared.FixedKeyIndex EncPRF.PermutationIndex) :
    lazyOnlineMachine.run (200663 + fuel) ⟨317604181, memory⟩ oracle =
      PMF.pure (some (⟨317804843, onlineFinalMemory memory⟩, oracle, 200663)) := by
  rw [Simulator.run_arithmetic_prefix, lazyOnlineMachine_finish_arithmetic, PMF.pure_bind]
  have halted : lazyOnlineMachine.code[317804843]'(by decide) = .compute .halt := by
    change lazyOnlineMachine.code[(317804843 : Fin 317804845).val] = _
    rw [lazyOnlineMachine_code]
    rfl
  simp only [show 200663 + fuel - 200663 + 1 = fuel + 1 by omega]
  rw [simulator_haltAt lazyOnlineMachine 317804843 halted fuel (onlineFinalMemory memory) oracle,
    PMF.pure_map]
  rfl
