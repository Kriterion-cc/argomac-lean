...
theorem lazyOnlineMachine_prepare [BN254.FieldCertificate] [BN254.GroupCertificate]
    ...
    FixedContinuation lazyOnlineMachine.arithmetic 22490 1560762 memory
      (onlinePreparedMemory memory output sample) (onlinePrepareCost sample) := by
  have targetSetupCode := lazyOnlineMachine_linearBefore onlineTargetSetup 22490 4 22494 (by decide)
    (onlineMachine_targetSetup 256) (by decide) (by decide)
  have retargetSetupCode := lazyOnlineMachine_linearBefore onlineRetargetSetup 26829 3 26832 (by decide)
    (onlineMachine_retargetSetup 256) (by decide) (by decide)
  have pointRetargetCode := lazyOnlineMachine_linearBefore retargetPointCode 26832 1533922 1560754 (by decide)
    (onlineMachine_pointRetarget 256) (by decide) (by decide)
  have linkSetupCode := lazyOnlineMachine_linear onlineLinkSetup
    (onlineBodyLabels 1560754 8 (by decide) 1560762) (onlineMachine_linkSetup 256) (by
      intro index inside
  ...
  have first : FixedContinuation lazyOnlineMachine.arithmetic 22490 22494 memory targetInitial 4 := by
    intro fuel
    exact linear_continue lazyOnlineMachine.arithmetic onlineTargetSetup
      (onlineBodyLabels 22490 4 (by decide) 22494) targetSetupCode memory fuel
  have targets : FixedContinuation lazyOnlineMachine.arithmetic 22494 26829 targetInitial
    ...
    have law := outputTargetsBlock_run lazyOnlineMachine.arithmetic
      (fun pc => onlineBodyLabels 22494 4335 (by decide) 26829 pc.val) lazyOnlineMachine_targets
    ...
    exact law
  have pointSetup : FixedContinuation lazyOnlineMachine.arithmetic 26829 26832
    ...
    exact linear_continue lazyOnlineMachine.arithmetic onlineRetargetSetup
      (onlineBodyLabels 26829 3 (by decide) 26832) retargetSetupCode
      (onlineTargetMemory memory output sample) fuel
  have pointRun : FixedContinuation lazyOnlineMachine.arithmetic 26832 1560754 retargetInitial retargeted 1533922 := by
    intro fuel
    exact retargetPointHost_continue lazyOnlineMachine.arithmetic
      (onlineBodyLabels 26832 1533922 (by decide) 1560754) pointRetargetCode retargetInitial fuel
  have linkSetup : FixedContinuation lazyOnlineMachine.arithmetic 1560754 1560762 retargeted
      (onlinePreparedMemory memory output sample) 8 := by
    intro fuel
    exact linear_continue lazyOnlineMachine.arithmetic onlineLinkSetup
      (onlineBodyLabels 1560754 8 (by decide) 1560762) linkSetupCode retargeted fuel
...
theorem lazyOnlineMachine_prepare_run [BN254.FieldCertificate] [BN254.GroupCertificate]
    ...
    lazyOnlineMachine.run (onlinePrepareCost sample + fuel) ⟨22490, memory⟩ oracle =
      (lazyOnlineMachine.run fuel ⟨1560762, onlinePreparedMemory memory output sample⟩ oracle).map
...
theorem lazyOnlineMachine_sampling_run [BN254.FieldCertificate]
    ...
    lazyOnlineMachine.run (onlineSamplingBudget 256 + fuel) ⟨13622, memory⟩ oracle =
      (onlineSamplingMemory 256 memory).bind fun sample =>
        (lazyOnlineMachine.run (onlineSamplingBudget 256 + fuel - sample.2) ⟨22490, sample.1⟩ oracle).map
  ...
  have sampled := onlineSamplingBlock_run lazyOnlineMachine.arithmetic 256
    (fun pc => onlineBodyLabels 13622 8868 (by decide) 22490 pc.val)
  ...
  change run lazyOnlineMachine.arithmetic (onlineSamplingBudget 256 + fuel) ⟨13622, memory⟩ =
    (onlineSamplingMemory 256 memory).bind (fun sample =>
      (run lazyOnlineMachine.arithmetic (onlineSamplingBudget 256 + fuel - sample.2)
        ⟨22490, sample.1⟩).map (Option.map fun final => (final.1, final.2 + sample.2))) at sampled
  ...
  | some outcome =>
    ...
    have positive := run_positive lazyOnlineMachine.arithmetic
      (onlineSamplingBudget 256 + fuel - sample.2) ⟨22490, sample.1⟩ next cost member
