...
def lazyParsedSetup [FieldCertificate] (machine : Simulator) (parameter : Nat) :
    PMF (Option (Pipeline.Table × Memory × LazyOracle.State Shared.FixedKeyIndex EncPRF.PermutationIndex)) :=
  (simulatorMemoryRun machine machine.firstFuel
    {bits := fun stack => if stack = 0 then [false, false] ++ natural parameter ++ natural 9806076 else []}
    LazyOracle.empty).map fun result => result.bind fun result =>
      (publicValue Wire.encoding 9806076 (result.1.bits 3)).map fun table => (table, result.1, result.2.1)
...
def lazyChosenDecision [FieldCertificate] [GroupCertificate] {Aux : Type}
    ...
    (memory : Memory) (table : Pipeline.Table)
  ...
  (LazyOracle.run (adversary.chooseInput parameter table auxiliary) oracle).bind fun selected =>
    (lazyParsedOnline machine memory selected.1.1
      (Shared.programCircuit.function scalar selected.1.1) selected.2).bind fun encoded => match encoded with
    | none => PMF.pure false
    | some encoded =>
...
theorem lazyParsedGame_phases [FieldCertificate] [GroupCertificate] {Aux : Type}
    ...
    (adversary : GarbledCircuit.AdaptiveAdversary sharedRealOracleSpec AffineInput Pipeline.Table GarbledCircuit.LamportSignature Aux)
    (parameter : Nat) (scalar : NonZeroScalar) (auxiliary : Aux) :
    GarbledCircuit.LazySimulatorProtocol.idealGame Shared.programCircuit Wire.encoding 9806076 machine
  ...
  | some setup =>
      cases parsed : publicValue Wire.encoding 9806076 (setup.1.memory.bits 3) with
