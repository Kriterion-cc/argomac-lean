...
theorem shared_lazy_real [BN254.FieldCertificate] [BN254.GroupCertificate]
    ...
    (adversary : AdaptiveAdversary (publicOracleSpec Shared.FixedKeyIndex EncPRF.PermutationIndex)
      BN254.AffineInput Pipeline.Table LamportSignature Aux)
    ...
    lazyRealGame (uniformRandomTape Shared.PrivateCoins (Shared.privateCoins witness))
      (fun p c r => (Shared.garbleProgram p c r).toOracleProgram) Shared.programCircuit.encode
  ...
  simp only [uniformRandomTape, uniformTape_eq, Shared.garbleProgram_correct, realGame]
  let experiment (tape : Shared.Randomness) : PMF Bool :=
    let garbled := Shared.wireCircuit.garble parameter scalar tape
    ((adversary.chooseInput parameter garbled.1 auxiliary).run
      (publicHandler Shared.evaluationOracle) tape).bind fun selected =>
      ((adversary.decide parameter garbled.1 (Shared.wireCircuit.encode garbled.2 selected.1.1)
  ...
  have stable : selected.2 = oracle := by
    have member : selected ∈ ((adversary.chooseInput parameter
        (Shared.programCircuit.garble parameter scalar (coins, oracle)).1 auxiliary).run
